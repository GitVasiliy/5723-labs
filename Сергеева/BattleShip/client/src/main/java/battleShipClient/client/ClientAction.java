...
import battleShipClient.common.*;
...
public class ClientAction {
    ...
    private void connect() {
        ...
        if (host.isEmpty()) {
            JOptionPane.showMessageDialog(mainWindow.getFrame(), Text.INVALID_HOST);
            return;
        }
        ...
        if (port == -1) {
            JOptionPane.showMessageDialog(mainWindow.getFrame(), Text.INVALID_PORT);
            return;
        }
        
        if (serverConnection.connect(host, port)) {
            menuPanel.lockMenu();
            mainWindow.displayStatus(Text.CONNECTED, true);
            serverConnection.prepareMessage(MessageType.HELLO);
            serverConnection.send();
        } else {
            mainWindow.displayStatus(Text.NOT_CONNECTED, true);
        }
    }
    ...
    private void disconnect() {
        ...
        mainWindow.displayStatus(Text.DISCONNECTED, true);
